 ...
 */
class PaymentDayTemplate extends RowTemplate
{
    ...
    protected function buildRow()
    {
        ...
        $row->addColumn($this->createColumn('date', 'Дата', 'getEmpty'));
        $row->addColumn($this->createColumn('title', 'Название проекта', 'getTitle', 
            CellStyle::create(null, null, 46.71)->setBorder(true)
        ));
        $row->addColumn($this->createColumn('pollRevenue', 
            'Доходы, стоимость успешной анкеты, руб', 'getPollRevenue'));
    ...
    public function getPollRevenue(Carbon $date, Project $project)
    {
        return $project->getRelation('lastProjectPeriod')->revenue;
    }

    public function getTitle(Carbon $date, Project $project)
    {
    ...
    private function getPaymentForOperatorFormula(Operator $operator, Carbon $date, Project $project)
    {
        ...
        return $strategy
            ->getProjectStrategy($project->id)
            ->getPayment(
                $strategy->getOperatorGroupRate($project), false
            );
    }
